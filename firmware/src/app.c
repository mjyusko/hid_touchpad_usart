...
void APP_PostProcessing(void) {
    uint8_t x = (uint8_t) appData.data.xMovement;
    uint8_t y = (uint8_t) appData.data.yMovement;

    appData.ppdata.MSSM_Report.byte1.d7 = 0; // Not 
    appData.ppdata.MSSM_Report.byte1.d6 = 1;

    if (appData.data.buttonState[0]) {
        /* Left Button - BUTTON1 pressed */
        appData.ppdata.MSSM_Report.byte1.d5 = 1; // LB
        LED1_On();
    } else {
        /* Left Button - BUTTON1 released */
        appData.ppdata.MSSM_Report.byte1.d5 = 0; // LB
        LED1_Off();
    }

    if (appData.data.buttonState[1])
        /* Right Button - BUTTON2 pressed */
        appData.ppdata.MSSM_Report.byte1.d4 = 1; // RB
    else
        /* Right Button - BUTTON2 released */
        appData.ppdata.MSSM_Report.byte1.d4 = 0; // RB

    appData.ppdata.MSSM_Report.byte1.d3 = // Y7
            (y & 0x80) >> 7;
    appData.ppdata.MSSM_Report.byte1.d2 = // Y6
            (y & 0x40) >> 6;
    appData.ppdata.MSSM_Report.byte1.d1 = // X7
            (x & 0x80) >> 7;
    appData.ppdata.MSSM_Report.byte1.d0 = // X6
            (x & 0x40) >> 6;

    appData.ppdata.MSSM_Report.byte2.d7 = 0; // Not used
    appData.ppdata.MSSM_Report.byte2.d6 = 0;
    appData.ppdata.MSSM_Report.byte2.d5 = // X5
            (x & 0x20) >> 5;
    appData.ppdata.MSSM_Report.byte2.d4 = // X4
            (x & 0x10) >> 4;
    appData.ppdata.MSSM_Report.byte2.d3 = // X3
            (x & 0x08) >> 3;
    appData.ppdata.MSSM_Report.byte2.d2 = // X2
            (x & 0x04) >> 2;
    appData.ppdata.MSSM_Report.byte2.d1 = // X1
            (x & 0x02) >> 1;
    appData.ppdata.MSSM_Report.byte2.d0 = // X0
            (x & 0x01) >> 0;

    appData.ppdata.MSSM_Report.byte3.d7 = 0; // Not Used
    appData.ppdata.MSSM_Report.byte3.d6 = 0;
    appData.ppdata.MSSM_Report.byte3.d5 = // Y5
            (y & 0x20) >> 5;
    appData.ppdata.MSSM_Report.byte3.d4 = // Y4
            (y & 0x10) >> 4;
    appData.ppdata.MSSM_Report.byte3.d3 = // Y3
            (y & 0x08) >> 3;
    appData.ppdata.MSSM_Report.byte3.d2 = // Y2
            (y & 0x04) >> 2;
    appData.ppdata.MSSM_Report.byte3.d1 = // Y1
            (y & 0x02) >> 1;
    appData.ppdata.MSSM_Report.byte3.d0 = // Y0
            (y & 0x01) >> 0;

    sprintf((char *) &appData.string[0], "%02X%02X%02X\r\n",
            appData.ppdata.MSSM_Report.byte[0],
            appData.ppdata.MSSM_Report.byte[1],
            appData.ppdata.MSSM_Report.byte[2]);
    appData.stringReady = true;
    appData.stringSize = 8; //64
    /* Clear the app data */
